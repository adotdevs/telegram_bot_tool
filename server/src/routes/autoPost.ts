...
r.post("/generate-ai", requireAuth, async (req: AuthedRequest, res) => {
  ...
  if (!cfg.openaiApiKey) {
    res.status(400).json({
      error:
        "OpenAI API key is not configured. Please add your OPENAI_API_KEY in Settings or in your .env file to enable AI generation.",
    });
    return;
  }
...
r.post("/:id/toggle", requireAuth, async (req: AuthedRequest, res) => {
  const doc = await AutoPostSchedule.findById(req.params.id);
  if (!doc) {
    res.status(404).json({ error: "Schedule not found" });
    return;
...
r.post("/:id/run-now", requireAuth, async (req: AuthedRequest, res) => {
  const doc = await AutoPostSchedule.findById(req.params.id);
  if (!doc) {
    res.status(404).json({ error: "Schedule not found" });
    return;
