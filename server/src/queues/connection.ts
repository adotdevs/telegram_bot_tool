...
if (url.startsWith("https://") || url.startsWith("http://")) {
  console.error(
    `[redis] ERROR: REDIS_URL is configured with HTTP(S) protocol: "${url.slice(0, 15)}...". ` +
    `BullMQ requires Redis TCP protocol. Please use the connection string starting with "rediss://" from the Upstash console.`
  );
}
...
export function createRedis() {
  ...
  client.on("error", (err: any) => {
    ...
    if (err?.code === "ENOTFOUND") {
      console.error(
        `[redis] DNS lookup failed for Redis host.\n` +
        `  -> If using Upstash:\n` +
