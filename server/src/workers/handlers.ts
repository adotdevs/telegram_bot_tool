...
import {
  nextActionDelayMs,
  longPauseMs,
  typingDelayMsForText,
  sleep,
  actionsBeforeLongPause,
...
function statsOf(
...
): NonNullable<CampaignDoc["stats"]> & { skipped: number } {
  if (!campaign.stats) {
    campaign.stats = {
      scraped: 0,
      added: 0,
  ...
  if (campaign.stats.skipped === undefined) campaign.stats.skipped = 0;
  return campaign.stats as NonNullable<CampaignDoc["stats"]> & { skipped: number };
}
...
async function maybeIdleBurst(): Promise<void> {
  const runtime = await getRuntimeSettings();
  if (!burstRangeReady) {
    untilLongPause = actionsBeforeLongPause(
      runtime.actionsBeforeLongPauseMin,
      runtime.actionsBeforeLongPauseMax
    );
...
export async function handleScrape(job: Job<{ campaignId: string }>): Promise<void> {
  ...
  if ("kind" in res) {
    if (res.kind === "flood") await markFloodWait(account._id, res.seconds);
    if (res.kind === "peer_flood") await markPeerFloodRisky(account._id);
    await logAction({
      campaignId: campaign._id,
  ...
  for (const u of res.users) {
    ...
    if (bl) continue;
    try {
      const r = await CampaignUser.updateOne(
        { campaignId: campaign._id, telegramId: u.telegramId },
        {
          $setOnInsert: {
            campaignId: campaign._id,
...
export async function handleAddUser(job: Job<{ campaignUserId: string }>): Promise<void> {
  const cu = await CampaignUser.findById(job.data.campaignUserId);
  if (!cu) return;
  const campaign = await Campaign.findById(cu.campaignId);
  if (!campaign || campaign.status !== "running") return;

  if (cu.status !== "queued" && cu.status !== "pending") return;
  ...
  await sendMessageQueue.add(
    "send",
    { campaignUserId: cu._id.toString() },
    { delay: nextActionDelayMs() + 3000 }
  );
}

export async function handleSendMessage(job: Job<{ campaignUserId: string }>): Promise<void> {
  const cu = await CampaignUser.findById(job.data.campaignUserId);
  if (!cu) return;
  const campaign = await Campaign.findById(cu.campaignId);
  if (!campaign || campaign.status !== "running") return;

  if (cu.status !== "added") return;
  ...
  const assigned =
    cu.assignedAccountId != null ? await TelegramAccount.findById(cu.assignedAccountId) : null;
  ...
  const text = await buildMessageText(
    campaign.messageTemplate,
    { first_name: firstName, username: firstName },
    campaign.useAiVariations ?? false
  );
  ...
  if (err && "kind" in err) {
    ...
    if (err.kind === "flood") {
      ...
      await sendMessageQueue.add(
        "send",
        job.data,
        { delay: Math.min(err.seconds * 1000, 3_600_000) }
      );
      return;
    }
    if (err.kind === "peer_flood") {
...
export async function handleCampaignTick(job: Job<{ campaignId: string }>): Promise<void> {
  const campaign = await Campaign.findById(job.data.campaignId);
  if (!campaign || campaign.status !== "running") return;

  const rs = await getRuntimeSettings();
  const wave = Math.min(
    campaign.batchSize ?? 75,
    rs.batchSizeMax,
    rs.campaignMaxParallel
  );
  const claimed: { _id: { toString: () => string } }[] = [];

  for (let i = 0; i < wave; i++) {
    const doc = await CampaignUser.findOneAndUpdate(
