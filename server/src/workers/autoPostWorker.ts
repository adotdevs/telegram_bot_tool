...
export async function handleAutoPost(job: Job<{ scheduleId: string }>): Promise<void> {
  ...
  let accounts: TelegramAccountDoc[] = [];
  ...
  const clientMap = new Map<string, Awaited<ReturnType<typeof createClientForAccount>>>();
 ...
 */
export async function resumeActiveAutoPostSchedules(): Promise<void> {
  try {
