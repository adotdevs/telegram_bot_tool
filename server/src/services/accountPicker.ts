...
export async function pickAccount(kind: "add" | "dm"): Promise<TelegramAccountDoc | null> {
  await TelegramAccount.updateMany(
    { status: "flood_wait", floodWaitUntil: { $lte: new Date() } },
    { $set: { status: "active" }, $unset: { floodWaitUntil: 1 } }
  );
  ...
  for (const acc of accounts) {
    if (acc.floodWaitUntil && acc.floodWaitUntil > new Date()) continue;
    if (kind === "add" && (await canAdd(acc))) return acc;
    if (kind === "dm" && (await canDm(acc))) return acc;
  }
  /** Fallback: any active not in flood if caps hit — still return null to delay job */
  return null;
}

export async function markFloodWait(accountId: unknown, seconds: number): Promise<void> {
...
export async function markPeerFloodRisky(accountId: unknown): Promise<void> {
  await TelegramAccount.findByIdAndUpdate(accountId, {
...
export async function clearFloodWaitIfExpired(account: TelegramAccountDoc): Promise<void> {
