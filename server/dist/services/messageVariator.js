...
export function synonymizeTemplate(template) {
    let out = template;
...
export function varyStructure(text) {
...
export async function maybeAiVary(baseText, useAi) {
    const cfg = await getRuntimeSettings();
    if (!useAi || !cfg.openaiApiKey)
        return baseText;
    try {
        const res = await fetch("https://api.openai.com/v1/chat/completions", {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {
                        role: "system",
                        content: "Rewrite ONE short DM in a natural human tone. Same intent and meaning. No markdown. Max 2 sentences.",
                    },
...
export function buildMessageText(template, vars, useAi) {
...
//# sourceMappingURL=messageVariator.js.map